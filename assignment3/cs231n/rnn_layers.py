...
def rnn_step_backward(dnext_h, cache):
    ...
    deriv_input_tanh = (1-next_h**2)*dnext_h

    #For bias, sum over the N dimension, like before
    db = deriv_input_tanh.sum(axis=0)

    #Same multiplication rules as affine layer above (and derived by hand before).
    #Multiplication node is a gradient swap operation
    dx = deriv_input_tanh.dot(Wx.T)
    dWx= x.T.dot(deriv_input_tanh)

    #and same order for h_{i-1} path...
    dprev_h = deriv_input_tanh.dot(Wh.T)
    dWh     = prev_h.T.dot(deriv_input_tanh)
